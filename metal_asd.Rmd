...
```{r earli models}
# model_fit <- function(name, pd){
#   asd.res <- list()
#   ntd.res <- list()
#   
#   sink(file.path(path,name))
#   for(met in metals){
#     sub = pd %>% filter(Analyte.Code==met)
#   
#     asd = sub %>% filter(asd %in% c('TD','ASD'))
#     asd$asd = factor(asd$asd)
#     ntd = sub %>% filter(asd %in% c('TD','Non-TD'))
#     ntd$asd = factor(ntd$asd)
#   
#     if(mod.type[met] == 'continuous'){
#   
#       #asd$Concentration_sg <- ifelse(asd$Concentration_sg<=0, NA, log(asd$Concentration_sg))
#       #ntd$Concentration_sg <- ifelse(ntd$Concentration_sg<=0, NA, log(ntd$Concentration_sg))
#   
#       fit.asd = glm(asd ~ Concentration_sg + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
#                     data=asd, family=binomial(link="logit"))
#       fit.ntd = glm(asd ~ Concentration_sg + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
#                     data=ntd, family=binomial(link="logit"))
#     }else{
#       
...
#                     data=asd, binomial(link="logit"))
#       fit.ntd = glm(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
#                     data=ntd, binomial(link="logit"))
#       
#       fit.asd = logbin(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, data=asd)
#       #fit.ntd = logbin(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, data=ntd, maxit=100000000)
#       
#       init.asd = glm(asd ~ GA_deliv_wks,
#                     data=asd, binomial(link="log"), start=c(0.5, -0.5))
#       fit.asd = glm(asd ~ Concentration_sg + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
#                     data=asd, poisson)
#     
#       ggplot(asd, aes(x=asd, y=GA_deliv_wks)) + geom_violin() + geom_jitter()
#     }
#   
#     asd.res[[met]] <- fit.asd
#     ntd.res[[met]] <- fit.ntd
#   
#     print(met)
#     print(summary(fit.asd))
#     print(IQR(asd$Concentration, na.rm=T))
#     print(summary(fit.ntd))
#     print(IQR(ntd$Concentration, na.rm=T))
#     print('')
#   }
#   sink()
#   
...
# earli.resT2 <- model_fit("EARLI_T2_logreg_estimates.txt", metals.earli.T2)
# earli.resT3 <- model_fit("EARLI_T3_logreg_estimates.txt", metals.earli.T3)
...
# earli.asd.T2 <- extract_res(earli.resT2$asd, metals.earli.T2, IQRs.T2)
# earli.ntd.T2 <- extract_res(earli.resT2$ntd, metals.earli.T2, IQRs.T2)
# 
# earli.asd.T3 <- extract_res(earli.resT3$asd, metals.earli.T3, IQRs.T3)
# earli.ntd.T3 <- extract_res(earli.resT3$ntd, metals.earli.T3, IQRs.T3)
# 
# write.csv(earli.asd.T2, file='Tables/earli_T2_logreg_asd.csv', row.names=F, na='')
# write.csv(earli.ntd.T2, file='Tables/earli_T2_logreg_ntd.csv', row.names=F, na='')
# write.csv(earli.asd.T3, file='Tables/earli_T3_logreg_asd.csv', row.names=F, na='')
# write.csv(earli.ntd.T3, file='Tables/earli_T3_logreg_ntd.csv', row.names=F, na='')
...
# marbles.resT2 <- model_fit("MARBLES_T2_logreg_estimates.txt", metals.marbles.T2)
# marbles.resT3 <- model_fit("MARBLES_T3_logreg_estimates.txt", metals.marbles.T3)
# 
# sink()
...
```{r}
# marbles.asd.T2 <- extract_res(marbles.resT2$asd, metals.marbles.T2, IQRs.mT2)
# marbles.ntd.T2 <- extract_res(marbles.resT2$ntd, metals.marbles.T2, IQRs.mT2)
# 
# marbles.asd.T3 <- extract_res(marbles.resT3$asd, metals.marbles.T3, IQRs.mT3)
# marbles.ntd.T3 <- extract_res(marbles.resT3$ntd, metals.marbles.T3, IQRs.mT3)
# 
# write.csv(marbles.asd.T2, file='Tables/marbles_T2_logreg_asd.csv', row.names=F, na='')
# write.csv(marbles.ntd.T2, file='Tables/marbles_T2_logreg_ntd.csv', row.names=F, na='')
# write.csv(marbles.asd.T3, file='Tables/marbles_T3_logreg_asd.csv', row.names=F, na='')
# write.csv(marbles.ntd.T3, file='Tables/marbles_T3_logreg_ntd.csv', row.names=F, na='')
...
pd.asd = pd %>% filter(asd %in% c('TD','ASD'))
pd.asd$asd = factor(pd.asd$asd)
pd.ntd = pd %>% filter(asd %in% c('TD','Non-TD'))
...
blood.asd = glm(asd ~ cd_bl + MomEdu + coi_gender + maternal_age + GA_deliv_wks, data=pd.asd, family=binomial(link="logit"))
...
blood.asd <- lapply(bl.metals, FUN=function(x){
  pd.asd[,x] <- log(pd.asd[,x],2)
  glm(paste0('asd ~ ', x, ' + MomEdu + coi_gender + maternal_age + GA_deliv_wks') %>% formula(), 
      data=pd.asd, family=binomial(link="logit")) %>%
    summary()
})

blood.ntd <- lapply(bl.metals, FUN=function(x){
  glm(paste0('asd ~ ', x, ' + MomEdu + coi_gender + maternal_age + GA_deliv_wks') %>% formula(), 
      data=pd.ntd, family=binomial(link="logit")) %>%
    summary()
})
...
grab_values <- function(summ){
  ...
  OR = lapply(summ, FUN=function(x){x$coefficients[2,1]}) %>% unlist
  err = lapply(summ, FUN=function(x){x$coefficients[2,2]}) %>% unlist
  OR = OR * blood.IQR
