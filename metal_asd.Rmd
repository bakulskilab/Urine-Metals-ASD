...
library(logbin)
library(epitools)
path <- 'L:/My Drive/Urine_Metals/Urine-Metals-ASD'
```
...
Skew
Arsenic Barium Manganese Mercury Molybdenum Tin

Slight
Cesium Cobalt Copper Nickel Selenium Thallium Zinc




Antimony Cadmium Chromium Lead Platinum Tungsten Beryllium Uranium Vanadium
...
load(file.path(path,'Data/dat_for_models.RDA'))
...
### top/bottom quartile of ASD PGS
asd.pgs <- read.table(file.path(path,'Data/marbles.qc.asd.allscores.txt'), header=T)
asd.pgs$coi_id <- substr(asd.pgs$id,1,6)
table(asd.pgs$coi_id %in% metals.marbles.T2$child_id)
table(asd.pgs$coi_id %in% metals.marbles.T3$child_id)

asd.pgs.T2 <- asd.pgs[asd.pgs$coi_id %in% metals.marbles.T2$child_id,]
asd.pgs.T3 <- asd.pgs[asd.pgs$coi_id %in% metals.marbles.T3$child_id,]

asd.pgs.T2$pgs_asd_q <- cut(asd.pgs.T2$pt1, breaks=quantile(asd.pgs.T2$pt1)) %>% as.factor() %>% as.numeric
asd.pgs.T3$pgs_asd_q <- cut(asd.pgs.T3$pt1, breaks=quantile(asd.pgs.T3$pt1)) %>% as.factor() %>% as.numeric

metals.marbles.T2$pgs_asd <- asd.pgs.T2[match(metals.marbles.T2$child_id, asd.pgs.T2$coi_id), 'pgs_asd_q']
metals.marbles.T3$pgs_asd <- asd.pgs.T3[match(metals.marbles.T3$child_id, asd.pgs.T3$coi_id), 'pgs_asd_q']
...
print('CESIUM')
print('Quartile 1 ASD PGS')
marbles.resT2.q1$Cesium
print('Quartile 4 ASD PGS')
marbles.resT2.q4$Cesium
