...
```{r dx_alg}
table(urine.metals$fam %in% marbearl$famid) #all T

marbearl.metal <- marbearl[marbearl$famid %in% urine.metals$fam, ]
...
#marbles diag first
urine.metals$asd <- ifelse(urine.metals$cohort=='MARBLES',marbearl.metal[match(urine.metals$child_id, marbearl.metal$coi_id),'Dx_alg'] , NA)
...
write.csv(metals.asd, file=file.path(here(),'Tables/asd_metals.csv'), row.names = F)

LOD.descriptives <- urine.nomulti %>% group_by(Analyte.Code, cohort, LOD, Time) %>% 
...
```{r}
pdf(file.path(here(),'../Plots/metal_correlation_earliT_early.pdf'))
...
export2word(tab.earli.T2, file=file.path(here(),'../Tables/EARLI_early_table1.docx'))
export2word(tab.earli.T3, file=file.path(here(),'../Tables/EARLI_late_table1.docx'))
...
pdf(file.path(here(),'../Plots/metal_correlation_marblesT_early.pdf'))
...
write.csv(outliers, file=file.path(path,'Tables/outliers_excluded_count.csv'), row.names=F, quote=F)
```
...
#save(metals.earli.T2, metals.earli.T3, metals.marbles.T2, metals.marbles.T3, mod.type, metals, IQRs.T2, IQRs.T3, IQRs.mT2, IQRs.mT3, file=file.path(path,'Data/dat_for_models.RDA'))
load(file.path(here(),'../Data/dat_for_models.RDA'))
...
model_fit_logbin <- function(pd, logistic=F){
  ...
  for(met in metals){
    ...
    if(mod.type[met] == 'continuous'){
      if(!logistic){
        fit.asdglm = glm(asd ~ Concentration_imp_sg_log2 + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
               data=asd, family=binomial)
        fit.asd = probratio(fit.asdglm, scale='log', method='delta')
        
        fit.ntdglm = glm(asd ~ Concentration_imp_sg_log2 + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
                 data=ntd, family=binomial)
        fit.ntd = probratio(fit.ntdglm, scale='log', method='delta')
      }else{
        fit.asd = glm(asd ~ Concentration_imp_sg_log2 + MomEdu + coi_gender + maternal_age + GA_deliv_wks,
                     data=asd, family=binomial)
        fit.ntd = glm(asd ~ Concentration_imp_sg_log2 + MomEdu + coi_gender + maternal_age + GA_deliv_wks,
                     data=ntd, family=binomial)
      }
    }else{
      ...
      if(!logistic){
        fit.asdglm = glm(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
               data=asd, family=binomial)
        fit.asd = probratio(fit.asdglm, scale='log', method='delta')
        
        fit.ntdglm = glm(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
                 data=ntd, family=binomial)
        fit.ntd = probratio(fit.ntdglm, scale='log', method='delta')
      }else{
...
cadmium lod exploration
```{r}

cadmium.earli <- metals.earli.T2 %>% filter(Analyte.Code=="Cadmium")
cadmium.earli$overLOD <- cadmium.earli$Concentration > cadmium.earli$LOD
cadmium.earli$overLOD2 <- cadmium.earli$Concentration > 0.12

table(cadmium.earli$asd, cadmium.earli$overLOD)
table(cadmium.earli$asd, cadmium.earli$overLOD2)
table(cadmium.earli$asd, cadmium.earli$LOD)
table(cadmium.earli$asd, cadmium.earli$LOD) %>% prop.table


lod06 <- cadmium.earli %>% filter(LOD==0.06)
lod12 <- cadmium.earli %>% filter(LOD==0.12)
summary(lod06$Concentration)
summary(lod12$Concentration)

cadmium.earli$over20 <- cadmium.earli$Concentration > 0.21
table(cadmium.earli$asd, cadmium.earli$over20)

cadmium.earli$LOD <- as.factor(cadmium.earli$LOD)

 
asd = cadmium.earli %>% filter(asd %in% c('TD','ASD'))
asd$asd = ifelse(asd$asd=='TD',0,1)
ntd = cadmium.earli %>% filter(asd %in% c('TD','Non-TD'))
ntd$asd = ifelse(ntd$asd=='TD',0,1)

fit.asdglm = glm(asd ~ LOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
               data=asd, family=binomial)
fit.asd = probratio(fit.asdglm, scale='log', method='delta')
        
fit.ntdglm = glm(asd ~ LOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks, 
                 data=ntd, family=binomial)
fit.ntd = probratio(fit.ntdglm, scale='log', method='delta')
...
cadmium.marbles <- metals.marbles.T2 %>% filter(Analyte.Code=="Cadmium")
cadmium.marbles$overLOD <- cadmium.marbles$Concentration > cadmium.marbles$LOD
cadmium.marbles$overLOD2 <- cadmium.marbles$Concentration > 0.12

table(cadmium.marbles$asd, cadmium.marbles$overLOD)
table(cadmium.marbles$asd, cadmium.marbles$overLOD2)
table(cadmium.marbles$asd, cadmium.marbles$LOD)
table(cadmium.marbles$asd, cadmium.marbles$LOD) %>% prop.table


lod06 <- cadmium.marbles %>% filter(LOD==0.06)
lod12 <- cadmium.marbles %>% filter(LOD==0.12)
summary(lod06$Concentration)
summary(lod12$Concentration)

cadmium.marbles$over20 <- cadmium.marbles$Concentration > 0.21
table(cadmium.marbles$asd, cadmium.marbles$over20)

cadmium.marbles$LOD <- as.factor(cadmium.marbles$LOD)

 
asd = cadmium.marbles %>% filter(asd %in% c('TD','ASD'))
asd$asd = ifelse(asd$asd=='TD',0,1)
ntd = cadmium.marbles %>% filter(asd %in% c('TD','Non-TD'))
ntd$asd = ifelse(ntd$asd=='TD',0,1)
...
# Quartiles of ASD PGS subset

```{r}
model_fit_mini <- function(pd, logistic=F){
  asd.res <- list()
  
  for(met in metals){
    asd = pd %>% filter(Analyte.Code==met)
    asd$asd = ifelse(asd$asd=='TD',0,1)
...
marbles.resT2.q1 <- model_fit_mini(metals.marbles.T2[metals.marbles.T2$pgs_asd==1,])
marbles.resT2.q4 <- model_fit_mini(metals.marbles.T2[metals.marbles.T2$pgs_asd==4,])

earli.resT2.q1 <- model_fit_mini(metals.earli.T2[metals.earli.T2$pgs_asd==1,])
earli.resT2.q4 <- model_fit_mini(metals.earli.T2[metals.earli.T2$pgs_asd==4,])

print('CADMIUM')
print('Quartile 1 ASD PGS')
marbles.resT2.q1$Cadmium
print('Quartile 4 ASD PGS')
marbles.resT2.q4$Cadmium

asd = metals.marbles.T2 %>% filter(Analyte.Code=='Cadmium')
asd$asd = ifelse(asd$asd=='TD',0,1)
asd$overLOD <- asd$Concentration > asd$LOD
fit.asdglm = glm(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks + overLOD*pgs_asdv, 
               data=asd, family=binomial)
fit.asd = probratio(fit.asdglm, scale='log', method='delta')
fit.asd
...
print('CADMIUM')
print('Quartile 1 ASD PGS')
earli.resT2.q1$Cadmium
print('Quartile 4 ASD PGS')
earli.resT2.q4$Cadmium

asd = metals.earli.T2 %>% filter(Analyte.Code=='Cadmium')
asd$asd = ifelse(asd$asd=='TD',0,1)
asd$overLOD <- asd$Concentration > asd$LOD
fit.asdglm = glm(asd ~ overLOD + MomEdu + coi_gender + maternal_age + GA_deliv_wks + overLOD*pgs_asdv, 
               data=asd, family=binomial)
...
save(res.asdT2.logit, res.asdT3.logit, res.ntdT2.logit, res.ntdT3.logit,
     file=paste0(path,'/Data/logistic_results.RDA'))
...
library(gridExtra)
library(openxlsx)
path <- 'G:/My Drive/Urine_Metals/Urine-Metals-ASD'

#load(file=paste0(path,'/Data/logistic_results.RDA'))
...
### counting N
N.eT2 <- metals.earli.T2 %>% filter(Analyte.Code=="Lead")
N.eT3 <- metals.earli.T3 %>% filter(Analyte.Code=="Lead")
N.mT2 <- metals.marbles.T2 %>% filter(Analyte.Code=="Lead")
N.mT3 <- metals.marbles.T3 %>% filter(Analyte.Code=="Lead")

res.asdT2$N <- ifelse(res.asdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("ASD","TD")), sum(N.mT2$asd%in%c("ASD","TD")))
res.asdT3$N <- ifelse(res.asdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("ASD","TD")), sum(N.mT3$asd%in%c("ASD","TD")))
res.ntdT2$N <- ifelse(res.ntdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("Non-TD","TD")), sum(N.mT2$asd%in%c("Non-TD","TD")))
res.ntdT3$N <- ifelse(res.ntdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("Non-TD","TD")), sum(N.mT3$asd%in%c("Non-TD","TD")))

res.asdT2$N.asd <- ifelse(res.asdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("ASD")), sum(N.mT2$asd%in%c("ASD")))
res.asdT3$N.asd <- ifelse(res.asdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("ASD")), sum(N.mT3$asd%in%c("ASD")))
res.ntdT2$N.asd <- ifelse(res.ntdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("Non-TD")), sum(N.mT2$asd%in%c("Non-TD")))
res.ntdT3$N.asd <- ifelse(res.ntdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("Non-TD")), sum(N.mT3$asd%in%c("Non-TD")))

res.asdT2$N.td <- ifelse(res.asdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("TD")), sum(N.mT2$asd%in%c("TD")))
res.asdT3$N.td <- ifelse(res.asdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("TD")), sum(N.mT3$asd%in%c("TD")))
res.ntdT2$N.td <- ifelse(res.ntdT2$cohort=="EARLI", sum(N.eT2$asd%in%c("TD")), sum(N.mT2$asd%in%c("TD")))
res.ntdT3$N.td <- ifelse(res.ntdT3$cohort=="EARLI", sum(N.eT3$asd%in%c("TD")), sum(N.mT3$asd%in%c("TD")))
...
meta_plots <-function(coefs, name){
  ...
  ggsave(combi.plot, file=paste0(path,'/Plots/meta/forest_',name,'.svg'),height=10, width=16)
}
...
write.xlsx(compiled.tab, file=paste0(path,'/Tables/meta_estimates_log_epitools_tables.xlsx'))

#write.xlsx(compiled.tab, file=paste0(path,'/Tables/meta_estimates_logistic_tables.xlsx'))
